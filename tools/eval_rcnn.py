...
def save_kitti_format(sample_id, calib, bbox3d, kitti_output_dir, scores, img_shape):
    ...
    with open(kitti_output_file, 'w') as f:
        for k in range(bbox3d.shape[0]):
            ...
            print("Scores",scores[k],np.argmax(scores[k]))
            print('%s -1 -1 %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f' %
                  (cfg.CLASSES+str(np.argmax(scores[k])), alpha, img_boxes[k, 0], img_boxes[k, 1], img_boxes[k, 2], img_boxes[k, 3],
                   bbox3d[k, 3], bbox3d[k, 4], bbox3d[k, 5], bbox3d[k, 0], bbox3d[k, 1], bbox3d[k, 2],
                   bbox3d[k, 6], max(scores[k])), file=f)
            print('%s -1 -1 %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f' %
                  (cfg.CLASSES+str(np.argmax(scores[k])), alpha, img_boxes[k, 0], img_boxes[k, 1], img_boxes[k, 2], img_boxes[k, 3],
                   bbox3d[k, 3], bbox3d[k, 4], bbox3d[k, 5], bbox3d[k, 0], bbox3d[k, 1], bbox3d[k, 2],
                   bbox3d[k, 6], max(scores[k])))
...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        if rcnn_cls.shape[2] == 1:
            ...
            # Pred classes torch.Size([400])
            # cls norm scores torch.Size([4, 100, 3])
            # Raw scores torch.Size([4, 400, 3])
            # norm scores torch.Size([4, 400, 3])
